...
import com.google.ai.edge.gallery.ui.smartobjectask.SmartObjectAskScreen // for smartobjectask
...
class LlmChatTask @Inject constructor() : CustomTask {
  ...
  override fun initializeModelFn(
    ...
    // 임시로 Ask Image의 초기화 방식을 그대로 사용
    val askImageTask = task.copy(id = BuiltInTaskId.LLM_ASK_IMAGE)
...
// Ask audio.
  ...
  class LlmSmartObjectAskTask @Inject constructor() : CustomTask {
    override val task: Task =
      Task(
        id = BuiltInTaskId.LLM_SMART_OBJECT_ASK,
        label = "Muse:Eye",
        category = Category.LLM,
        icon = Icons.Outlined.Mms, // 일단 같은 아이콘 사용
        models = mutableListOf(),
        description = "Docent at everywhere, whenever you want",
        docUrl = "https://ai.google.dev/edge/mediapipe/solutions/genai/llm_inference/android",
        sourceCodeUrl =
          "https://github.com/google-ai-edge/gallery/blob/main/Android/src/app/src/main/java/com/google/ai/edge/gallery/ui/llmchat/LlmChatModelHelper.kt",
        textInputPlaceHolderRes = R.string.text_input_placeholder_llm_chat,
    ...
    override fun MainScreen(data: Any) {
      val myData = data as CustomTaskDataForBuiltinTask
      // 새로 만든 SmartObjectAskScreen 사용
      SmartObjectAskScreen(
        modelManagerViewModel = myData.modelManagerViewModel,
        navigateUp = myData.onNavUp,
      )
    }
  ...
  internal object LlmSmartObjectAskModule {
    @Provides
    @IntoSet
    fun provideTask(): CustomTask {
      return LlmSmartObjectAskTask()
    }
  }
}
