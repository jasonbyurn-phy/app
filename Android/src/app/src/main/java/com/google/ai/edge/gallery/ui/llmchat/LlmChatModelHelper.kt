...
object LlmChatModelHelper {
  ...
  fun runInference(
    ...
    // 🆕 시스템 프롬프트 추가 gy
    val enhancedInput = if (images.isNotEmpty() || audioClips.isNotEmpty()) {
      // 이미지/오디오가 있을 때만 시스템 프롬프트 추가
...
// 🆕 대화식 프롬프트 생성 gy
private fun buildConversationalPrompt(userInput: String): String {
  // 사용자가 이미 "답변 규칙"을 포함했는지 확인
  if (userInput.contains("답변 규칙") || userInput.contains("간결하게")) {
    return userInput // 이미 있으면 그대로 사용
  }
...
답변 시 다음을 지켜주세요:
- 3-4문장으로 핵심만 간결하게 답변
- 사용자가 추가 질문을 할 수 있도록 대화식으로 답변
- 불필요한 배경 설명은 생략
- 답변 마지막에는 "다른 궁금한 점 있으신가요?"를 추가"""
}
