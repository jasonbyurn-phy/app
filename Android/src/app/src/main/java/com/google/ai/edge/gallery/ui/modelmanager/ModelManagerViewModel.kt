    ...
    for (task in
      getTasksByIds(
        ids =
          setOf(
            ...
            BuiltInTaskId.LLM_SMART_OBJECT_ASK, // 이 줄 추가
      ...
      if (
        (task.id == BuiltInTaskId.LLM_ASK_IMAGE && model.llmSupportImage) ||
          ...
          (task.id == BuiltInTaskId.LLM_SMART_OBJECT_ASK && model.llmSupportImage) || // 이 줄 추가
          (task.id != BuiltInTaskId.LLM_ASK_IMAGE && task.id != BuiltInTaskId.LLM_ASK_AUDIO && task.id != BuiltInTaskId.LLM_SMART_OBJECT_ASK) // 마지막 && 추가
      ) {
  ...
  fun loadModelAllowlist() {
    ...
    viewModelScope.launch(Dispatchers.IO) {
      try {
        ...
        // Process all tasks.
        processTasks()
        // Smart Object Ask에 Ask Image 모델들 복사
        val askImageTask = curTasks.find { it.id == BuiltInTaskId.LLM_ASK_IMAGE }
        val smartObjectAskTask = curTasks.find { it.id == BuiltInTaskId.LLM_SMART_OBJECT_ASK }
        if (askImageTask != null && smartObjectAskTask != null) {
          for (model in askImageTask.models) {
            smartObjectAskTask.models.add(model)
          }
        }
        // Update UI state.
  ...
  private fun createUiState(): ModelManagerUiState {
    ...
    for (importedModel in dataStoreRepository.readImportedModels()) {
      ...
      if (model.llmSupportImage) {
        ...
        tasks.get(key = BuiltInTaskId.LLM_SMART_OBJECT_ASK)?.models?.add(model) // 이 줄 추가
      }
