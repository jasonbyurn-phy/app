...
BASE_DIR = Path("D:/gallery_unity/Android/src/app/src/main/assets/")
OUTPUT_FILE = BASE_DIR / "embeddings" / "all_embeddings_kim.bin"
IMAGE_SIZE = 256
EMBEDDING_DIM = 512
# ===== 1. 모델 로드 바로 아래에 추가 =====
def find_image_output_index(output_details):
    import numpy as np

    # 1) shape / dtype 기준 후보 수집: (1,512) 또는 **512 포함** + float 출력
    candidates = []
    for i, od in enumerate(output_details):
        shape = tuple(int(x) for x in od["shape"])
        dt = od["dtype"]
        if 512 in shape and dt in (np.float32, np.float16):
            candidates.append(i)
    ...
    for i, od in enumerate(output_details):
        name = od["name"].lower()
        if any(k in name for k in ["image", "img", "vision"]):
            return i

    # 3) 그래도 못 찾으면 0으로 (대부분 단일 출력이거나 첫 출력이 이미지)
    return 0
...
# 모델 로드 후 출력 정보가 찍힌 다음에:
IMAGE_OUTPUT_INDEX = find_image_output_index(output_details)
IMAGE_OUTPUT_INDEX = 1
print(f"🧭 IMAGE_OUTPUT_INDEX = {IMAGE_OUTPUT_INDEX}")
